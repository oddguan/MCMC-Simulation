...
let mcmc = (g, r, T, NumVertices, iteration) => {
  /*
  ...
  */
  let chain = new Map()
  let flag = true
  let thetai, newG, thetaj, piRatio, qRatio, acceptP, aOD, result, sOld, sNew
  sOld = Math.floor(Math.random() * NumVertices)
  for (let i = 0; i < iteration; i++) {
    console.log(`iteration ${i + 1}`)
    thetai = calculateTheta(r, g, sOld)
    ...
    sNew = Math.floor(Math.random() * NumVertices)
    thetaj = calculateTheta(r, newG, sNew)
...
let main = (args) => {
  ...
  */
  ...
  let chain = mcmc(g, args.r, args.Temperature, args.NumVertices, args.iteration)
  let sum = 0
  chain.forEach((value, key, map) => {
